import type { InstitutionItem } from "@/api/types";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Label } from "@/components/ui/label";
import { Plus, Trash2, ChevronUp, ChevronDown, Landmark, X, Upload } from "lucide-react";
...
type InstitutionsEditorProps = {
  ...
  onChange: (items: InstitutionItem[]) => void;
  token: string;
};

export function AdminInstitutionsEditor({ institutions, onChange, token }: InstitutionsEditorProps) {
  const list = institutions.length ? institutions : [];

  function setAt(i: number, next: InstitutionItem) {
    const copy = [...list];
    copy[i] = next;
    onChange(copy);
  }
    ...
    <div className="space-y-5">
      <div className="overflow-hidden rounded-[1.75rem] border border-gold/15 bg-[linear-gradient(140deg,rgba(255,205,86,0.08),rgba(255,205,86,0.02))] shadow-[0_24px_70px_rgba(0,0,0,0.14)]">
        <div className="flex flex-wrap items-end justify-between gap-3 p-5">
          <div>
            <Label className="flex items-center gap-2 text-base font-display font-semibold text-foreground">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl gold-gradient-bg shadow-lg shadow-gold/20">
                <Landmark className="h-4 w-4 text-black" aria-hidden />
              </div>
              Institutions
            </Label>
          ...
          <Button
            ...
            onClick={() => onChange([...list, emptyInstitution()])}
          >
      ...
      {list.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gold/20 bg-gradient-to-b from-gold/5 to-transparent px-6 py-12 text-center">
          <Landmark className="h-8 w-8 text-gold/40 mx-auto mb-3" />
          <p className="text-sm text-muted-foreground">
            No institutions yet. Click <span className="text-gold font-semibold">Add Institution</span> to add one.
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="pl-3 space-y-4">
                ...
                <div className="space-y-2 pl-3">
                  <Label className="text-xs font-semibold text-foreground/80 flex items-center gap-2">
                    <span className="w-1.5 h-1.5 bg-gold/60 rounded-full"></span>
                    Videos <span className="text-muted-foreground font-normal">(optional)</span>
                  </Label>
                  <Textarea
                    rows={2}
                    placeholder="Enter video URLs, one per line"
                    className="min-h-[3rem] border-gold/20 bg-black/20 resize-y focus:border-gold/50 focus:ring-gold/20"
                    value={(item.videos || []).join("\n")}
                    onChange={(e) =>
                      setAt(i, {
                        ...item,
